...
export class SecurityInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if (request.url.indexOf(environment.serverMethodsUrl) != 0 && request.url.indexOf(environment.dbHopeKurseTextbausteine) != 0 && request.url.indexOf(environment.securityUrl) != 0) {
